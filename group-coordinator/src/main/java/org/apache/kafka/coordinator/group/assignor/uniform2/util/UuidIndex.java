...
/**
 * The dense indices of a fixed set of {@link Uuid}s: the ids get the indices {@code 0} to
 * {@code n - 1} in the order given, and are looked up in open addressing tables of their two
 * longs, so a lookup hashes and compares longs without boxing. It is much cheaper than a
 * {@code HashMap<Uuid, Integer>} when lookups are done in large numbers.
 */
public final class UuidIndex {
    ...
    /**
     * @param ids The ids, which get the indices {@code 0} to {@code ids.length - 1}. They must be
     *            distinct.
     */
    public UuidIndex(Uuid[] ids) {
        var capacity = Integer.highestOneBit(Math.max(2, ids.length * 2 - 1)) << 1;
        ...
        for (int i = 0; i < ids.length; i++) {
            var msb = ids[i].getMostSignificantBits();
            var lsb = ids[i].getLeastSignificantBits();
            var slot = slot(msb, lsb);
    ...
    private int slot(long msb, long lsb) {
        var hash = (msb ^ lsb) * 0x9E3779B97F4A7C15L;
