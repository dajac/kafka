/*
 * Licensed to the Apache Software Foundation (ASF) under one or more
 * contributor license agreements. See the NOTICE file distributed with
 * this work for additional information regarding copyright ownership.
 * The ASF licenses this file to You under the Apache License, Version 2.0
 * (the "License"); you may not use this file except in compliance with
 * the License. You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package org.apache.kafka.coordinator.group.assignor.uniform2.util;

import java.util.Arrays;

/**
 * A minimal binary min-heap of longs, which avoids the boxing of a {@code PriorityQueue<Long>}:
 * values are stored in an array and pushed without allocation until the array is full, and the
 * smallest value is popped first.
 */
public final class LongArrayHeap {
    private long[] values;
    private int size;

    /**
     * @param capacity The initial capacity, which may be zero. The heap grows as needed.
     * @throws IllegalArgumentException If the capacity is negative.
     */
    public LongArrayHeap(int capacity) {
        if (capacity < 0) {
            throw new IllegalArgumentException("Negative capacity: " + capacity);
        }
        values = new long[capacity];
    }

    /**
     * @return Whether the heap has no value.
     */
    public boolean isEmpty() {
        return size == 0;
    }

    /**
     * @return The number of values.
     */
    public int size() {
        return size;
    }

    /**
     * Removes all the values, keeping the capacity.
     */
    public void clear() {
        size = 0;
    }

    /**
     * Adds the value, growing the heap when it is full.
     */
    public void push(long value) {
        if (size == values.length) {
            values = Arrays.copyOf(values, Math.max(8, size * 2));
        }
        var i = size++;
        while (i > 0) {
            var parent = (i - 1) >>> 1;
            if (values[parent] <= value) {
                break;
            }
            values[i] = values[parent];
            i = parent;
        }
        values[i] = value;
    }

    /**
     * @return The smallest value, which is removed from the heap.
     * @throws IllegalStateException If the heap is empty.
     */
    public long pop() {
        if (size == 0) {
            throw new IllegalStateException("The heap is empty");
        }
        var top = values[0];
        var last = values[--size];
        var i = 0;
        while (true) {
            var child = 2 * i + 1;
            if (child >= size) {
                break;
            }
            if (child + 1 < size && values[child + 1] < values[child]) {
                child++;
            }
            if (values[child] >= last) {
                break;
            }
            values[i] = values[child];
            i = child;
        }
        values[i] = last;
        return top;
    }
}
