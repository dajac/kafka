 ...
 *     the few that keep the other members balanced when the change of quotas leaves them
 ...
 *     <li>The <i>quota</i> of a subscriber for a topic is its number of base partitions, plus
 *     one if it gets an extra partition.</li>
 *     <li>The <i>load</i> of a member is the sum of its quotas, that is the number of partitions
 ...
 * </ul>
 *
 * <p><b>Key idea.</b> The algorithm decides quotas before partition ids. The spread property
 * holds by construction of the quotas, and the balance of the group only depends on which
 * subscribers get the extra partitions. So the problem reduces to distributing the extra
 * partitions, which are few, and partition ids are only handled once the quotas are final,
 * one topic at a time. Stickiness comes from letting members keep the extra partitions and
 * the partition ids they currently have whenever the quotas allow it.
 *
 ...
 *
 * <p><b>Phase 4, partitions.</b> Every topic is handled on its own, with the quotas now final.
 * Each current holder keeps its current partitions up to its quota, the lowest partition ids
 * first, and releases the rest. The partitions that nobody kept are then handed out, in
 * ascending order, to the members below their quota: the current holders of the topic first,
 ...
 * an extra partition receive one. A topic where every holder already has exactly its quota,
 * and the holders together have every partition, is emitted as is. A member whose partitions
 ...
 * Partitions:
 *   T1, quotas A 2, B 1, C 2:  A gets 0 and 1, B gets 2, C gets 3 and 4.
 *   T2, quotas A 1, B 2, C 1:  A gets 0, B gets 1 and 2, C gets 3.
 * </pre>
 ...
 * Partitions:
 *   T1, quotas A 2, B 1, C 1, D 1:  A keeps 0 and 1, B keeps 2, C keeps 3 and releases 4,
 *                                   D receives 4.
 *   T2, quotas 1 for everyone:      A keeps 0, B keeps 1 and releases 2, C keeps 3,
 *                                   D receives 2.
 ...
 * Partitions:
 *   T1, quotas A 1, B 2:  A keeps 0 and releases 1 and 2, which B receives.
 *   T2, quotas A 2, B 1:  A keeps 0 and 1 and releases 2, which B receives.
 ...
 * algorithm. It never changes the quotas, so the properties above are kept as they are. It
 ...
 * <ol>
 *     <li><b>Keep:</b> each current holder keeps its current aligned partitions up to its
 *     quota. Misaligned partitions are released so that they can be realigned, and come back
 *     to their holder later if they cannot be. A holder with more aligned partitions than its
 *     quota releases first the ones most useful to the racks whose members are below their
 *     quotas, then the ones with the most replica racks, which are the easiest to place.</li>
 *     <li><b>Align:</b> the released partitions are grouped by the set of racks holding their
 *     replicas, and the members below their quota are grouped by rack. The largest number of
 ...
 * and partition ids beyond the current partition count of a topic, are dropped.
 *
